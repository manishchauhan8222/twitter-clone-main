...
function TweetBox() {
    ...
    const handleTweet = async (e) => {
        ...
        if (user?.providerData[0]?.providerId === 'password') {
            try {
                const response = await axios.get(`https://twitter-clone-aevo.onrender.com/loggedInUser?email=${email}`)
            ...
            // fetch(`https://twitter-clone-aevo.onrender.com/loggedInUser?email=${email}`)
            //     .then(res => res.json())
            //     .then(data => {
            //         setName(data[0]?.name)
        ...
        if (name) {
            ...
            try {
                const data = await axios.post('https://twitter-clone-aevo.onrender.com/post', userPost)
                console.log(data)
            } catch (error) {
                console.log(error.response);
                if(error.response.status === 403){
                    alert('Posting limit reached for your subscription plan.')
                }
            }
            setPost('')
            setImageURL('')
